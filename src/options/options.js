...
$('exportAll').addEventListener('click', async () => {
  const [settings, profile, resume, answerBank] = await Promise.all([
    RA.storage.getSettings(), RA.storage.getProfile(), RA.storage.getResume(), RA.storage.getAnswerBank()
  ]);
  // The API key is deliberately left out of the export.
  const data = { profile, resume, answerBank, settings: Object.assign({}, settings, { apiKey: '' }) };
...
$('importFile').addEventListener('change', async (ev) => {
  ...
  try {
    ...
    await Promise.all([loadResume(), loadProfile(), loadSettings(), loadBank()]);
